...
const addLoopImgs = async (req, res, next) => {
  ...
  if (data.id){
    ...
    const endData = await GifRooms.updateData(data.id,data.path,cover,data.from,data.title,data.des,"待审核",1,req.body.userid);
    if (endData){
    ...
    const endData = await GifRooms.insertData(data.path,cover,data.from,data.title,data.des,"待审核",1,req.body.userid);
    if (endData){
