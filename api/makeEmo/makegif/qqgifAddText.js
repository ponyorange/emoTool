...
const getLoopImgs = async (req, res, next) => {
  ...
  if (+userScroe < makeBaseEmoScroe){//用户不够积分
    ...
    for (var i = 0; i < texts.length; i++){
      textString = textString + texts[i].text;
    }
    checkoutText(textString,req,res,gifGenerator,texts,userId,userScroe);
  }
};

//***检验文字***//
function checkoutText(text,req,res,gifGenerator,texts,userId,userScroe) {
  var  getTokenOption = {
    method: 'get',
